<template>
  <div class="rounded-lg  flex items-center justify-evenly w-full h-full relative bg-white md:w-screen md:h-screen md:bg-[#194bfb] overflow-hidden">
    <div class="rounded-md w-full h-full flex items-center justify-center overflow-hidden">
      ...
      >
        <div class=" text-lg">
          <div class="font-sans text-4xl font-bold text-center mb-4">Ladder Admin</div>
          <p class="text-gray-600 mb-2">初始化须知</p>
          <p class="text-gray-600 mb-2">1.使用前请确保已阅读 README 文档</p>
          <p class="text-gray-600 mb-2">2.数据库初始化完成后，需要重启一次才能启用所有功能</p>
          <p class="text-gray-600 mb-2">3.推荐使用Mysql8，请确认数据库引擎为<span class="text-red-600 font-bold text-3xl ml-2 ">innoDB</span></p>
...
const form = reactive({
  dbType: 'mysql',
  host: 'ladder-admin-mysql',
  port: '3306',
  userName: 'la-user',
  password: 'ladder-admin@2024',
  dbName: 'ladder-admin',
  dbPath: ''
})
