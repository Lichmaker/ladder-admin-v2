...
func (s *SystemApi) SetSystemConfig(c *gin.Context) {
	...
	if len(sys.Config.V2RayManager.SecretKey) > 0 && !utils.CheckAESKeyValid(sys.Config.V2RayManager.SecretKey) {
		response.FailWithMessage("加密key只能使用数字和英文且长度为16/24/32", c)
		return
	}
