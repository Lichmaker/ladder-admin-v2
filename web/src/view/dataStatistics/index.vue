  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        >
          <template #default="scope">
            {{ dayjs(scope.row.updatedAt).tz().format('YYYY-MM-DD HH:mm:ss') }}
          </template>
        </el-table-column>
        <el-table-column
          align="left"
...
import { batchGetUserInfoByEmail } from '@/api/user'
...
const getTableData = async() => {
  ...
  if (table.code === 0) {
    ...
    for (var i = 0, len = table.data.list.length; i < len; i++) {
      userEmailArr.push(table.data.list[i].email)
    }
    const userMapData = await batchGetUserInfoByEmail(userEmailArr)
    for (var i2 = 0, len2 = table.data.list.length; i2 < len2; i2++) {
      const userInfo = userMapData.get(table.data.list[i2].email)
      if (!userInfo) {
...
onMounted(() => {
  if (route.query.email) {
    searchInfo.value.email = route.query.email
  }
  getTableData()
})
