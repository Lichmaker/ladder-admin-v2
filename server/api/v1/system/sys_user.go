...
func (b *BaseApi) BatchQuery(c *gin.Context) {
	var condition systemReq.BatchQueryUser
	...
	query, err := userService.BatchQuery(condition)
	if err != nil {
		response.FailWithMessage(fmt.Sprintf("查询失败：%s", err), c)
		return
	}
